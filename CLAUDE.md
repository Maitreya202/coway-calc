...
- **admin.html은 기존 행을 검색해서 수정하는 도구일 뿐, 새 제품(행)을 만드는 기능은 없음** — 별매품을 새로 등록하려면 먼저 구글 시트 `제품DB`에 직접 새 행을 추가해야 함(제품군(s)="별매품", 모델명/제품명/약정년별 가격 등). 그 다음에야 admin.html 검색·"별매품 연결" UI에서 찾을 수 있음.
...
- 별매품페어링 시트는 **"메인모델 ↔ 별매품모델" 연결 관계만** 저장 (가격은 저장 안 함 — 중복 저장하면 제품DB 쪽 가격만 고치고 여기는 안 고쳐서 어긋나는 문제가 생기기 때문. `타사보상지로` 필드가 정확히 이 이유로 드리프트가 생겼던 전례가 있음):
  `대상모델명 | 별매품모델명 | 1개당여부` (1행 헤더)
  - `code.gs`의 `_parseBmpPairings()`가 헤더 텍스트로 컬럼을 찾아 모델명별로 묶어서 `getConditions().bmpPairings`로 반환 → `{대상모델명: [{모델명:별매품모델명, 개당:bool}, ...]}`
- `index.html`의 `itemBmpOptions(item)`이 `BMP_PAIRINGS[item.모델명]`의 각 연결에 대해 **그 별매품 모델명으로 PRODUCTS를 조회**해서 item의 약정년/재렌탈여부에 맞는 가격을 그때그때 가져옴(정상가/프로모션가는 `hasPromo` 판정과 동일한 방식으로 결정) — 페어링 시트에는 가격 컬럼이 아예 없음.
  - 일시불 구매 모드엔 별매품 자체가 적용 안 되는 구조라 일시불가는 신경 안 씀(cash 분기가 `calcItem()` 초반에 별도 early return).
- `renderDiscPanels()`가 이 옵션 목록을 체크박스 여러 개로 렌더링하고, 선택 상태는 `item.별매품선택 = {별매품제품명: {on, cnt}}`에 저장. `calcItem()`은 선택된 옵션들의 가격(1개당 옵션은 수량 곱해서)을 전부 합산.
...
- **약정 없이 일시불로만 파는 별매품(수압펌프 등 부속품)도 지원함(2026-08-05~)** — 제품DB에 이런 제품을 등록할 땐 약정(년) 칸을 비워두거나 "-"로 두고 일시불가만 채우면 됨. `code.gs`의 `_fetchData()`가 약정(년)이 없어도 일시불가가 있으면 약정년=0(termless 표식)으로 응답에 포함시킴(전에는 약정(년) 없는 행이 통째로 빠졌었음 — 별매품 등록 후 검색 안 되는 문제의 원인이었음). `_updateProduct()`의 행 매칭도 이 경우(시트 셀이 빈칸/"-"→NaN)를 0으로 취급해서 admin.html 저장이 되도록 맞춰둠.
  - `index.html`의 `itemBmpOptions()`이 연결된 별매품의 제품DB 행을 보고 **정상가/프로모션/재렌탈이 전부 0이고 일시불가만 있으면** 자동으로 "일시불 전용"(`oneTime:true`)으로 판정 — 별도 플래그 없이 가격 필드 유무로만 구분함.
