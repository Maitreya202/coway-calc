...
- **`타사보상지로` 필드는 계산 로직에서 더 이상 사용 안 함(deprecated)** — 2026-08-04에 "지로납부" 공통 토글로 대체됨. 시트/컬럼 자체는 남아있고 admin.html에서도 여전히 조회·수정 가능하지만, `index.html`의 `calcItem()`은 이 값을 읽지 않음. 실데이터 검증 결과 대부분(94%)의 행에서 `타사보상지로 - 타사보상렌탈 = 900원`으로 일정했고, 이는 "자동납부 시 1,000원 할인 → 지로면 미할인, 단 고정액은 % 할인 적용 전에 더해짐"이라는 사용자 설명과 일치함(예외 28개 행은 4개 특정 모델의 데이터 오류로 추정, `APD-1023A`/`APD-1025E`/`APM-1226H`/`AP-1225F`).
