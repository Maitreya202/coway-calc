...
- 각 페어링의 `할인적용` 플래그(true/false/null=미설정)가 이 별매품이 메인과 같이 살 때 % 할인(동시구매/결합/일시불할인 등)에 같이 낄지 결정 — **렌탈/일시불 문맥마다 미설정 시 기본값이 다름**(기존 동작을 안 깨기 위함): 렌탈은 `할인적용 !== false`(미설정=할인대상, 기존처럼 항상 할인 스택에 낌), 일시불은 `할인적용 === true`(미설정=할인 미적용, 원가 그대로 더해짐). 일시불 모드에서 할인대상인 별매품 줄은 메인과 동일한 -10%/-15%/-10%+10% 할인이 그 줄에도 개별 적용됨(`calcItem()`의 `cashDiscApply()`).
