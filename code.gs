...
// ----------------------------------------------------------------
// updateCardDataSep2026() ← "제휴카드_기본"/"제휴카드_구간"/"제휴카드_유의사항"
// 시트를 2026년 9월 제휴카드 프로모션 공지(제휴카드 9월.xlsx) 기준으로
// 전체 재작성합니다. Apps Script 에디터에서 한 번만 실행하면 됩니다.
//
// 9월 변경 요약:
//  - 신한/우리/삼성/KB/NH: 금액 동일, 기간만 9.1~9.30으로 갱신
//  - 현대카드 M Edition3(hyundaiM): 8월과 동일 조건으로 계속 진행, 기간만 갱신
//    (현대카드 플레인(hyundai)은 이번 달도 프로모션 없음, N 유지)
//  - NH농협카드(nhbank): 카드명만 "코웨이 NH올원카드"로 변경(카드ID/연락처 등은 유지),
//    프로모션 내용은 8월과 동일
//  - 롯데카드(loca): 9월 16일부터 재신청 가능 프로모션으로 목록에 복귀
//    (60개월간 최대 8천원 추가 청구할인)
// ----------------------------------------------------------------
function updateCardDataSep2026() {
  var ss = SpreadsheetApp.openById(SPREADSHEET_ID);

  var baseRows = [
    ['카드ID','카드명','문의번호','프로모션여부','혜택개월수','기간','대상','프로모션제목','특이사항','연회비','신청안내','신청URL'],
    ['shinhan','코웨이 신한카드','1833-6013','Y',60,'2026.9.1 ~ 9.30','직전 6개월간 신한카드(신용카드) 청구 및 사용 이력이 없는 고객','60개월간 최대 3만원 청구할인','','국내 27,000원 / 해외 30,000원','QR코드 또는 ☎ 1833-6013','https://shcard.io/coway'],
    ['hyundai','코웨이 현대카드','1670-0443','N',60,'','','','전국 코스트코에서 결제 가능, 애플페이 이용 가능','국내/해외 20,000원','☎ 1670-0443',''],
    ['woori','코웨이 ICON 우리카드','1800-0859','Y',60,'2026.9.1 ~ 9.30','직전 6개월간 우리카드(신용카드) 청구 및 사용 이력이 없는 고객','60개월간 최대 3만원 청구할인','','국내/해외 25,000원','☎ 1800-0859',''],
    ['samsung','코웨이 삼성카드','1588-7540','Y',60,'2026.9.1 ~ 9.30','직전 6개월간 삼성카드(신용카드) 청구 및 사용 이력이 없는 고객','60개월간 월 최대 1만 5천원 추가 할인','본인/가족 통신비·아파트 관리비·4대 사회보험 등 생활 요금 정기 결제 시, 결제 건별 월 1,000원(최대 3,000원) 추가 할인','국내/해외 15,000원','☎ 1588-7540',''],
    ['loca','코웨이 X LOCA (롯데카드)','1577-5208','Y',60,'2026.9.16 ~ 9.30','직전 6개월간 롯데카드(신용카드) 청구 및 사용 이력이 없는 고객','60개월간 최대 8천원 추가 청구할인','','국내/해외 20,000원','6개월 무실적 대상 확인 및 카드 발급 페이지 · ☎ 1577-5208','https://m.lottecard.co.kr/app/LPBNFDA_V580.lc?evnBultSeq=1&ccoEvnGubun=coway'],
    ['kb','KB국민 코웨이Ⅱ카드','1644-8388','Y',36,'2026.9.1 ~ 9.30','직전 6개월간 KB국민카드(KB BC카드 포함) 청구 및 사용 이력이 없는 고객','36개월간 최대 3만원 청구할인','','국내/해외 30,000원','카드 신청 페이지(QR코드) · ☎ 1644-8388',''],
    ['nhbank','코웨이 NH올원카드','1644-2866','Y',60,'2026.9.1 ~ 9.30','직전 6개월간 NH올원카드(신용카드) 청구 및 사용 이력이 없는 고객','60개월간 1만원 추가 캐시백 프로모션','','국내 10,000원 / 국내외 12,000원','☎ 1644-2866','https://go.nhcard.com/GscljMo'],
    ['hana','마이 코웨이 하나카드','1588-1771','N',60,'','','','','','☎ 1588-1771',''],
    ['ibk','코웨이 IBK카드','1566-0088 → 14#','N',60,'','','','','','☎ 1566-0088 → 14#',''],
    ['nhall1','코웨이 NH올원카드','1644-2866 → 6#','N',60,'','','','','','☎ 1644-2866 → 6#',''],
    ['hyundaiM','코웨이 현대카드 M Edition3','02-2655-5003','Y',36,'2026.9.1 ~ 9.30','직전 6개월간 현대카드(신용카드) 청구 및 사용 이력이 없는 고객','36개월간 월 1만원 추가 청구할인','전국 코스트코에서 결제 가능, 애플페이 이용 가능','국내/해외 30,000원','QR코드 또는 ☎ 02-2655-5003',''],
  ];
  ...
  var tierRows = [
    ...
    ['woori','30만원 이상',13000,11000,''], ['woori','80만원 이상',17000,7000,''], ['woori','150만원 이상',23000,7000,''],
    ['samsung','30만원 이상',7000,15000,''], ['samsung','70만원 이상',10000,14000,''], ['samsung','120만원 이상',13000,13000,''],
    ['loca','40만원 이상',8000,8000,''], ['loca','80만원 이상',12000,7000,''], ['loca','160만원 이상',24000,'',''],
    ['kb','40만원 이상',15000,11000,''], ['kb','80만원 이상',20000,10000,''],
    ['nhbank','30만원 이상',10000,10000,'캐시백'], ['nhbank','100만원 이상',15000,10000,'캐시백'], ['nhbank','200만원 이상',30000,10000,'캐시백'],
    ['hana','30만원 이상',13000,'',''], ['hana','80만원 이상',18000,'',''], ['hana','150만원 이상',25000,'',''],
    ['ibk','30만원 이상',13000,'',''], ['ibk','70만원 이상',17000,'',''], ['ibk','120만원 이상',23000,'',''],
  ...
  var noteRows = [
    ...
    ['samsung',4,'삼성카드 신규 발급 고객 대상 (재발급, 기보유 회원, 해지 후 1년 미경과 회원 제외 · 본인 카드만)'],
    ['samsung',5,'코웨이 렌탈료 면제 프로모션 고객은 면제 회차에 청구할인 혜택 미적용'],
    ['loca',1,'2026년 9월 16일부터 신청 가능'],
    ['loca',2,'코웨이 렌탈/멤버십료 자동이체 필수'],
    ['loca',3,'카드 발급 후 카드 이용실적 충족 및 자동납부 매출발생 필수'],
    ['loca',4,'60개월간(카드 유효기간까지) 최대 8천원 청구할인 제공'],
    ['loca',5,'코웨이 렌탈료 면제 프로모션 고객은 면제 회차에 청구할인 혜택 미적용'],
    ['kb',1,'코웨이 렌탈/멤버십료 자동이체 필수'],
    ['kb',2,'자동납부 건당 결제금액이 전월실적 40만원 구간은 15,000원 이상, 80만원 구간은 20,000원 이상 결제 시 프로모션 적용 가능'],
    ['kb',3,'렌탈료 면제 프로모션 고객도 혜택 적용 가능 (렌탈료가 발생하는 월부터 청구할인 적용되며, 자동납부 매출 발생 시 36개월간 적용)'],
    ['nhbank',1,'렌탈 자동이체 매출 발생 필수'],
  ...
  CacheService.getScriptCache().remove('appData');
  return '제휴카드 9월 데이터 갱신 완료 (기본/구간/유의사항 3개 탭 재작성)';
}
...
// 할인적용여부: "일시불 구매"로 이 별매품을 살 때만 의미 있는 플래그 — Y면 메인 제품과 같은 -10%/-15%/-10%+10% 할인이
// 그 별매품 줄에도 적용됨(index.html의 discountableCash). 렌탈 별매품은 이 값과 무관하게 항상 동시구매/결합 등
// %할인 스택에 포함됨(calcItem()에 하드코딩) — 렌탈 쪽은 현재 페어링 단위로 껐다 켰다 할 수 없음.
// 헤더가 없거나 셀이 비어있으면 "미설정"(null)이고, 일시불 문맥에서는 미설정=할인 미적용(false)으로 처리됨.
